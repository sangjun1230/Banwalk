					...
					<div class="row">
						...
						<div class="col-lg-9">
							...
							<div style="width: 700px; text-align: center;">
								<input value="${result}"> <br>
							</div>

								<form action="<%=root%>/bupdatecontent.do?no=${result.no }" method="POST">
									<input type="submit" class="btn btn-dark" value="수정">
								</form>
								<br>
								<form action="<%=root%>/deletecontent.do?no=${result.no }" method="POST">
							...
							<script>
								...
								function fnreply(){
									$.ajax({
										type : "GET",
										url : "<%=root%>/replyinsert.do",
										data : $("#boardreply").serialize(),
										dataType : "text",
										error : function() {
											alert("통신실패!!");
										},
							...
							<p>
							<form id="boardreply" method="GET">
								<input type="hidden" name="no" value="${result.no}" readonly="readonly"> 
									<input type="text" name="r_n_id" value="${member.n_id}"> 
									<input type="text" name="r_contents" placeholder="댓글을 작성하세요.">
								<div onclick="fnreply()" class="btn btn-primary">작성</div>
							</form>
							</p>
							<div id="commentList"></div>
							<div id="commentDelete"></div>
							<div>
