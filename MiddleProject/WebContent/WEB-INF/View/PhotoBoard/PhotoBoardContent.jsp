...
<head>
  ...
  function getReplyList(){
			$.ajax({
				type:"GET",
				url:"photoboardreply.do?no=${vo.no}",
				dataType:"text",
				error:function(){
					alert("통신실패!!");
				},
...
<body id="page-top">
  ...
  <div class="container">
  	  <div class="row">
  	  	<div class="col-md-2">제목 : ${vo.title}</div>
		<div class="col-md-2">닉네임 : ${vo.n_id}</div>
		<div class="col-md-1">조회수: ${vo.viewcount}</div>
	  </div>
	  <div class="row">
	  	<div class="col-md-4"><img src="<%=root%>/img/${vo.url}"></div>
		<div class="col-md-2">${vo.contents}</div>
		
		<c:if test="${vo.n_id == member.n_id}">
			<form action="<%=root%>/goupdate.do?no=${vo.no}" method="POST">
				<input type="submit" value="수정">
			</form>
			<form action="<%=root%>/getdelete.do?no=${vo.no}" method="POST">
		...
		<div id="reply">
 		...
 		<div>
 			<form id="reinsert" method="GET">
 				<input type="hidden" name="no" value="${vo.no}">
 				<input type="text" name="r_n_id" value="${member.n_id}" readonly="readonly">
 				<input type="text" name="r_contents" placeholder="댓글을 작성하세요.">
 				<div onclick="replyInsert()" class="btn btn-primary">작성</div>
 			</form>
 		</div>
  </div>
